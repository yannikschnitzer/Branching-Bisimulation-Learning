...
def get_initial_samples(num: Int, ts: BranchingTransitionSystem, mr = 0,r = 10):
    cexs = []
    for i in range(num):
        samples = (np.random.randint([dom[i][0] for i in range(dim)], [dom[i][1] + 1 for i in range(dim)], (nu1m, dim)))
