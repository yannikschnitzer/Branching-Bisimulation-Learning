...
def get_initial_samples(num: Int, ts: BranchingTransitionSystem, mr = 0,r = 10,dim=2):
    cexs = []
    for i in range(num):
        s = (np.random.randint([mr for _ in range(dim)], [r + 1 for _ in range(dim)], (dim)))
        w = (np.random.randint([mr for _ in range(dim)], [r + 1 for _ in range(dim)], (dim)))
        cexs.append(IntVal(s),IntVal(ts.successors(s)),IntVal(w))
    return cexs
