...
def get_initial_samples(num: Int, ts: BranchingTransitionSystem, mr = 0,r = 10,dim=2):
    cexs = []
    for i in range(num):
        s = [IntVal(i) for i in np.random.randint([mr for _ in range(dim)], [r + 1 for _ in range(dim)], (1,dim))]
        w = [IntVal(i) for i in np.random.randint([mr for _ in range(dim)], [r + 1 for _ in range(dim)], (1,dim))]
        cexs.append((s,ts.successors(s),w))
    return cexs
