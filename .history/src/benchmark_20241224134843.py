...
def compare_times(branching, deterministic, iters = 10, explicit_classes = True):
    ...
    for i in range(iters):
        ...
        print(f"--- Branching Implicit Formula {i}: {branching_times[-1]}s expired ")
    brn_avg = np.average(branching_times_impl)
    print(f"--- Branching Implicit Formulas - Average expired time is {brn_avg}s")
    ...
    for i in range(iters):
        ...
        print(f"--- Branching Explicit Formula {i}: {branching_times[-1]}s expired ")
    brn_avg = np.average(branching_times_expl)
    print(f"--- Branching Explicit Formulas - Average expired time is {brn_avg}s")
